...
export default function ClaudeToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      // Get key from dropdown, fallback to first key or sk_airouter for localhost
      const keyToUse = selectedApiKey?.trim() 
        || (apiKeys?.length > 0 ? apiKeys[0].key : null)
        || (!cloudEnabled ? "sk_airouter" : null);
          ...
          {!checkingClaude && claudeStatus?.installed && (
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!claudeStatus?.hasAIRouter} loading={restoring}>
